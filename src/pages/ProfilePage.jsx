...
export default function ProfilePage() {
  ...
  const [postingScanId, setPostingScanId] = useState(null);
  ...
  const postScanToCommunity = async (scanId) => {
    setPostingScanId(scanId);

    try {
      const data = await apiFetch(`/scans/${scanId}/community`, {
        method: 'POST',
        body: JSON.stringify({}),
      });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {formatDistanceToNow(new Date(item.createdAt), { addSuffix })}
                  </div>
                  {item.details && (
