...
function ThreadView({ post, onBack }) {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      ...
      try {
          const data = await apiFetch(`/thread/${post.id}/comments`);
        if (!ignore) {
...
const map = new Map();
    ...
    try {
      const data = await apiFetch(`/thread/${post.id}/comment`, {
        method: 'POST',
        body: JSON.stringify({
          comment,
          parentCommentId,
  ...
  const updateComment = async (commentId) => {
    ...
    try {
      const data = await apiFetch(`/thread/${post.id}/comment/${commentId}`, {
        method: 'PATCH',
...
function PostCard({
  ...
  const refreshCounts = async () => {
    const [voteData, commentData] = await Promise.all([
      ...
      apiFetch(`/thread/${post.id}/comments/count`),
    ]);
  ...
  useEffect(() => {
    ...
    try {
      const data = await apiFetch(`/thread/${post.id}/thread-likes`, {
        method: 'POST',
...
export default function Zone() {
  ...
  useEffect(() => {
    ...
    const fetchThreads = async () => {
      ...
      try {
        const data = await apiFetch('/created-threads');
  ...
  const saveThread = async () => {
    ...
    try {
      if (editorMode === 'edit' && editingPostId) {
        const data = await apiFetch(`/thread/${editingPostId}`, {
          method: 'PATCH',
          body: JSON.stringify({
            title,
  ...
  const toggleComments = async (post) => {
    try {
      const data = await apiFetch(`/thread/${post.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
  ...
  const deleteThread = async (post) => {
    try {
      await apiFetch(`/thread/${post.id}`, {
        method: 'DELETE',
      });
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-lg">
              <Card className="glass relative border-primary/50 shadow-[0_0_20px_rgba(0,255,255,0.1)]">
                ...
                <CardFooter className="flex justify-end gap-3">
                  ...
                  >
                    {isSavingThread ? 'Saving...' : mode === 'edit' ? 'Save Thread' : 'Broadcast Thread'}
                  </Button>
                </CardFooter>
              </Card>
            </motion.div>
